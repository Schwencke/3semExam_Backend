...
public class JWTAuthenticationFilter implements ContainerRequestFilter {
    Function<Integer, Integer> sd = x -> x* x;
 ...
 public void filter(ContainerRequestContext request) throws IOException {
   if (isSecuredResource()) {
     ...
     if (token == null) {
       request.abortWith(errorhandling.CustomExceptionMapper.makeErrRes("Not authenticated - do login", 403));
       return;
     }
     try {
       ...
       request.abortWith(errorhandling.CustomExceptionMapper.makeErrRes("Token not valid (timed out?)", 403));
     }
   }
 }
